...
export default class Search extends React.Component {
  ...
  getInfos() {
    return fetch("https://api.nasa.gov/insight_weather/?api_key=PAQslsDyD4gonPcnIngcaOv2w9wfJXYAysOEi5OY&feedtype=json" )
    .then((response) => response.json())
    .then((responseJson) => {
      this.setState({ infos : responseJson,
      solKeys: responseJson.sol_keys});
    })
    .catch((error) => {
      console.error(error);
    });
